...
1. **Already-reviewed guard.** Skip when this change has already met a reviewer:
   - The staged work came from `/subagent-implementation`, `/quick-fix`, `/autopilot`, or `/subagent-diagnose` in this session. Each dispatches `atomic-reviewer` per iteration, so the diff has been read by a fresh context already.
