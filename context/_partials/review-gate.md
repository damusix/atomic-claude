...
1. **Already-reviewed guard.** Skip when this change has already met a reviewer:
   ...
   - `atomic-verify`'s gate already dispatched `atomic-auditor` on this change in this session and the tree has not changed since.
...
3. **Dispatch** `atomic-auditor` (`subagent_type: "atomic-auditor"`) with `diff: working` and `intent:` (what the change was meant to do, and the spec path if one exists); the subject is the working tree, the same scope `atomic-verify`'s gate and the `atomic hooks stop` hook use, while step 2's docs-only guard keeps reading the staged set. There is no spec for ad-hoc work — say so rather than inventing one; the auditor judges the diff against the intent.
...
5. Report the totals line to the user and `audit: <verdict> (atomic-auditor on <model>)` before writing the commit message.

The main agent editing code directly is the third implementation path, alongside the subagent loop and `/quick-fix`. `/implement` is that path run deliberately, and it carries its own per-checkpoint gate; this step covers the same editing done ad-hoc, outside any command, with the same agent atomic-verify uses so the change meets one independent reader, not two. **Why:** the agent that wrote the code is the worst judge of it — it reviews its own reasoning rather than the diff, and every rationalization that produced the bug is still in its context.
</review-gate>{{- end}}
