...
3. **Dispatch** `atomic-reviewer` (`subagent_type: "atomic-reviewer"`) in code mode on the staged diff (`diff: staged`), with an explicit model override to the session's model and a one-line `intent:` describing what the change was meant to do. There is no spec for ad-hoc work — say so rather than inventing one; the `intent:` stands in for it.
...
5. Report the totals line and `review: <verdict> (atomic-reviewer on <model>)` to the user before writing the commit message.
