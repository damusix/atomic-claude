...
7. Create the PR:
    ```
    gh pr create --title "<type>(<scope>): <imperative, ≤70 chars>" --body <HEREDOC> [--attach '<path>#<caption>' ...]
    ```
    The title carries a Conventional Commit prefix whenever the project uses them, because a squash merge turns it into the commit subject on the base branch and a changelog generator reads nothing else. Confirm with `git log <base> --oneline -20`: conventional subjects there mean the prefix is required here. Type by user-visible impact across the whole PR — `feat:` for new behavior, `fix!:` for anything removed or renamed.
