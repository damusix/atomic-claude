...
3. Dispatch the `atomic-wiki-inferrer` agent in silent mode, passing the scope's steering so the silent refresh honors the user's corrections instead of falling back to pure inference:
   ```
   mode: silent
   first_run: false
