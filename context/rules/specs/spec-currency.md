...
Every spec body carries three sections beyond Goal/Non-goals/Success criteria/Checkpoints/Risks: `## Change tree` — an indented tree of files, one line per node, marked `A` created / `M` modified / `D` removed, with sketch-level symbol notes only — `## Outline` — a hollow outline of the work: per file, the named pieces to be created or reshaped (functions/types for code, sections/blocks for markdown artifacts; a mixed change uses each file's natural unit), one line each as `name — responsibility`, members nesting one level under their parent piece (a type's methods, a section's subsections) and no deeper, never signatures or bodies, or `None — <reason>` when the change has no nameable pieces — and `## Flows` — numbered actor → step sequences per behavior being implemented, or `None — <reason>` when the change ships no runtime behavior.

The tree and the outline go inside a code fence. Both carry their structure in indentation and column alignment, which a markdown renderer reflows away unless the block is preformatted; a bare fence with no language keeps a syntax highlighter off a file tree. A `None — <reason>` stands on its own without a fence — it is a sentence, not a block. Flows are never fenced: a bold flow name followed by an ordered list, as real markdown, so steps can carry inline code and links. A human approves the spec before any code exists; these sections are what make blast radius, shape, and behavior inspectable without reverse-engineering prose — and the outline is what the reviewer walks the delivered work against.

This applies forward only: it governs specs drafted after this rule ships. A pre-existing spec is not backfilled by an unrelated line-level amendment. Backfill happens only when a scope-changing amendment rewrites the body anyway — at that point the tree, outline, and flows describe the amended scope like every other section.

Amendments keep all three sections current — that is the body-is-current-truth rule above applied to these sections, not a new lifecycle rule.
