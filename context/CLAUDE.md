...
- Comment discipline. A comment states what the code can't show — a constraint, an invariant, a non-obvious why, a gotcha — in the fewest lines that carry it. Never narrate the next line, restate the diff, or address the reviewer ("fixed per review"). Point at docs rather than copying them: cite `docs/spec/<topic>.md` and stop, never quote a spec or paste its change log. Carry no plan or process residue — checkpoint IDs, issue numbers, dates, agent chatter. Match the file's existing comment density and idiom, and document new public APIs with the language's own docstring convention. **Why:** narration, reviewer-talk, and copied doc text are stale the moment they are written, and a stale comment misleads every reader who trusts it over the code.
