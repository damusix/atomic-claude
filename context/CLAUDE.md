...
2. **Implement** — `/subagent-implementation` reads the spec, runs the implement→review loop, commits per green iteration. (`/subagent-diagnose` for failure-driven work.) `/quick-fix` skips the plan for a known-cause fix with one obvious approach — same loop, no spec. `/implement` is the main-agent path run deliberately: for work whose context is already in the conversation, the main agent writes the code and `atomic-reviewer` gates every checkpoint, finalizing with a range-scoped signals refresh and one strong final gate the user picks (`atomic-auditor`, `atomic-strategist`, or `atomic-reviewer`). Editing directly with no command around it is the remaining path and the only one with no loop, so its review gate lives at the two exits instead: `atomic-verify` dispatches `atomic-reviewer` before the work is called ready, and the ship verbs dispatch it before the commit lands.
