...
- [`output-styles/atomic.md`](../../../output-styles/atomic.md) — governs Claude's TUI reply style. Terse, structure-over-prose, fragments OK. Reframed around clarity (substance stays, fluff dies); intensity levels removed. Includes an explicit `# Subagents` section noting that agents carry the response-voice rule directly via `agent-atomic-voice` partial — no need to brief them for terseness. Applied to main agent only.
