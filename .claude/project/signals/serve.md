...
- [`atomic/internal/serve/`](../../../atomic/internal/serve) — leaf package; 32 files. Key files:
  ...
  - `graph.go` — `BuildLinkGraph` walks all `*.md` files under root, extracts links via `mdlink.ExtractLinks`, and resolves wikilinks using nearest-then-alphabetical rule; `Edge` struct with `Broken`, `Ambiguous`, `CodeFile` (non-.md source files rendered as `/file/` links rather than broken), `External` flags; `Graph` carries `nodeSet map[string]bool` for O(1) `Has()` membership; `resolvePageHref` is the render-time link-rewrite counterpart to `resolveMarkdownLink`; `resolveDirIndex` probes [`README.md`](../../../README.md) / [`readme.md`](../../../readme.md) / `index.md` / [`.claude/project/signals.md`](../signals.md) in that order
  - `rail_handler.go` — `NewRailHandler` for `GET /rail/<relpath>`; renders four htmx OOB fragments: `#rail-props-content` (YAML frontmatter key/values in source order via `frontmatter.ParseOrdered`; scalars pass through as-is, `[]any` elements are comma-joined, nested maps are skipped; slot CSS-hidden when empty), `#rail-out-content` (outbound links with broken/codefile/ambiguous/external annotations), `#rail-in-content` (backlinks + orphan note), `#rail-graph-content` (Cytoscape mini-graph seeded by `/graph/data?node=<page>&depth=1`); `cyID` is derived from the relpath with slashes/dots replaced by hyphens for valid HTML id; uses `data-rail-graph-url` attribute + `htmx.onLoad` pattern (inline `<script>` in OOB swaps unreliable in htmx 2); path-traversal guard + graph-membership O(1) check via `g.Has(rel)` both enforced before reading the file
...
- `GET /rail/<relpath>` emits four OOB fragments (not three): `#rail-props-content` (YAML frontmatter properties), `#rail-out-content`, `#rail-in-content`, `#rail-graph-content`. The properties slot is hidden via CSS when the page has no frontmatter.
- Right-rail mini-graph uses `data-rail-graph-url` attribute + `htmx.onLoad` delegation for Cytoscape init; inline `<script>` tags in OOB `innerHTML` swaps are not reliably executed by htmx 2. `data-rail-graph-url` is intentionally distinct from the `[system]` toggle's click-handler attribute to avoid selector collision.
