...
- **Agent consumption**: all keystone agents (investigator, builder, surgeon, reviewer, haiku, signals-inferrer) now carry the `agent-code-intel` partial. The partial contract: index present → prefer `atomic code search/callers/callees/impact`; bounded queries only (one symbol); graceful degradation to sg/grep on any failure (binary absent, DB missing, query error) — never surface errors about the index being unavailable.
