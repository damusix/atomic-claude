...
Commands: 32 top-level `.md` files. Agents roster: 9. `atomic/internal/` now has 21 subdirectories (added: `bundlespec/`, `prompt/`, `updatedoctor/`, `followups/` relative to the prior-prior snapshot). `doctor/` remains at 35 files (`checks_followups.go` rewritten — old regex-on-markdown parser replaced by `followups.LoadEntriesWithErrors`; two new repair fns registered). `docs/spec/` grew to 15 files: added `follow-ups-folder.md`. `docs/design/` at 5 files. `.claude/project/followups.md` deleted; replaced by `.claude/project/followups/` folder (11 per-entry files + `INDEX.md` + `CLOSED.md`). `.claude/skills/atomic-cli-contrib/SKILL.md` exists as a project-local contributor skill (not bundled). `docs/credits.md` at `docs/` root. `assets/atomic-claude.png` (logo, not bundled).
...
- **Planned: documentation skill split**: `docs/spec/documentation-skill-split.md` specs a new `atomic-documentation` skill that owns surface taxonomy and voice routing; `/documentation` becomes a thin orchestrator. Ship verbs would auto-invoke the skill between signals refresh and commit synthesis. Not yet implemented - spec only.
