---
generated_at: 2026-05-17T23:15:00Z
...
Commands: 26 top-level `.md` files (up from 24). New additions: `commands/commit-and-push.md` (`/commit-and-push` — commit then push, no PR, no merge; trunk-based counterpart to `/commit-and-pr`) and `commands/push-only.md` (`/push-only` — push existing commits, no commit, no PR; trunk-based counterpart to `/pr-only`). Both auto-bundle via the existing `commands/*.md` inclusion rule; bundle item count is now 46 (up from 44). Skills roster: 7 (`atomic-commit`, `atomic-debug`, `atomic-review`, `atomic-signals`, `atomic-tdd`, `atomic-verify`, `atomic-prose`). `docs/design/`: 2 design docs (`atomic-doctor.md`, `atomic-validate.md`). New at repo root: `LICENSE`, `.githooks/pre-commit`.
