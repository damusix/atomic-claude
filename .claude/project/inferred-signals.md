---
generated_at: 2026-05-17T23:55:00Z
...
Commands: 29 top-level `.md` files (up from 26). New additions since last scan: `commands/session-report.md` (`/session-report` — capture per-session *why* context for multi-session branches; consumed by ship verbs during commit-message synthesis, then deleted), `commands/review-branch.md` (`/review-branch`), `commands/undo-commit.md` (`/undo-commit`), plus earlier additions `commit-and-push.md` and `push-only.md`. All auto-bundle via the existing `commands/*.md` inclusion rule; bundle item count is now 47 total items. `docs/spec/` grew to 7 files with the addition of `session-report.md`. Skills roster: 7 (`atomic-commit`, `atomic-debug`, `atomic-review`, `atomic-signals`, `atomic-tdd`, `atomic-verify`, `atomic-prose`). `docs/design/`: 2 design docs (`atomic-doctor.md`, `atomic-validate.md`). New at repo root: `LICENSE`, `.githooks/pre-commit`.
