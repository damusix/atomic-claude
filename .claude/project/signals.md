...
**User profile feature (post-merge 16377eb)**: `atomic/internal/profile/` (`profile.go`, `profile_test.go`) captures env at install time (`CaptureEnv`) and generates `~/.claude/.atomic/profile.md` stub (`RenderStub`). Sections: Identity/Work/Active projects/Interests/People mentioned (`<stable>`/`<volatile>`) + Environment (`<deterministic>` with git user.name, user.email, OS, arch, CPU count). Doctor category 10 (`checks_profile.go`) verifies the file exists and `@~/.claude/.atomic/profile.md` is wired in a candidate CLAUDE file. `config.ProfilePath` / `config.ProfileRelPath` in `atomic/internal/config/paths.go` are the canonical path derivation. Spec: `docs/spec/user-profile.md`. Design: `docs/design/user-profile.md`.
