...
Both `context/commands/` AND `context/agents/` are **fully generated** from `templates/` via `make render`. Never edit `context/commands/<name>.md` or `context/agents/<name>.md` directly — the change is overwritten on the next render. The rendered kinds and their order are defined in `templaterender.renderedKinds` (`["commands", "agents"]`).
...
The pre-commit hook auto-runs `make render` and re-stages `context/commands/` and `context/agents/` whenever any `templates/` file is staged.
