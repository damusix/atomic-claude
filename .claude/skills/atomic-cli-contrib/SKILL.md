...
This skill is contributor-scope only. It lives under `.claude/skills/` and is auto-loaded for sessions in this repo. It is not bundled (`MatchesSkillDir` in `atomic/internal/bundlespec/bundlespec.go` admits only `context/skills/atomic-*/` into the canonical corpus).
...
- The embedded bundle is a gitignored build artifact, regenerated by `make -C atomic bundle` (a prerequisite of `build`, `test`, and `vet`). Nothing to stage, and no pre-commit stage for it. See `.githooks/pre-commit`.


## 10. Command, agent, and steering artifacts — one file each, under `context/`


A command, agent, or the global steering source is a single committed file: `context/commands/<name>.md`, `context/agents/<name>.md`, or `context/AGENTS.md`. There is no separate template and no generated copy. Edit the file directly.


A source may compose a reusable block with `{{ template "<name>" . }}`, resolved against `context/_partials/`. One pool serves every expanding kind. Expansion happens inside `make bundle`, on the way into the embedded bundle — nothing is written back into `context/`.
...
|------|---------|-------------|
| Command big partials | `commit-flow`, `pr-flow`, `merge-flow`, `squash-flow`, `push-flow` | Entire flow bodies consumed by one or more commands |
...
| Agent partials (`agent-` prefix) | `agent-tdd-signals`, `agent-signals-output`, `agent-shared-rules` | Blocks shared verbatim across implementer + reviewer (TDD workflow, signal output format, discipline rules) |


**Adding one:** create the single file. **Removing one:** delete the single file.


**Partial design rules:**

- Pure fragments only. No `dict` function, no `{{ if }}` conditionals, no variant flags inside partials. When two consumers diverge by one word, generalize the wording so the partial stays verbatim-shared, or keep that bullet inline per-artifact.
...
- A directive naming an undefined partial fails `make bundle`, so a typo surfaces at build time rather than shipping a hole.
- Commands, agents, and `context/AGENTS.md` (the global steering source) expand. Skills, rules, and output styles pass through byte-for-byte, so a literal `{{` in their prose is safe.
...
7. **Citing an `atomic` CLI flag or verb in an artifact without verifying it against the binary.** Before writing `atomic <verb> --some-flag` into any command, agent, skill, doc, or `CLAUDE.md`, confirm the spelling against `atomic <verb> --help` from a binary **built from local source** (`go build -o /tmp/atomic ./cmd/atomic` run inside `atomic/`), not the globally-installed `atomic` (which lags your branch) and not memory. This is how `atomic code … --format json` shipped across six agent prompts when the real flag is `--json` — a wrong flag in an artifact fails silently at author time and only surfaces when a user runs the example. The binary's own `--help` is the source of truth; artifacts must match it. **`atomic validate artifacts` now automates this check** — it is wired into bare `atomic validate` and the CI step, so wrong-flag citations in committed artifacts fail CI. Run `atomic validate artifacts` locally after editing any artifact that cites a CLI flag.
