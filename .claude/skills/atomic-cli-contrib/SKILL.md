...
## 10. Command & agent artifacts — one file each, under `context/`


A command or agent is a single committed file: `context/commands/<name>.md` or `context/agents/<name>.md`. There is no separate template and no generated copy. Edit the file directly.


A source may compose a reusable block with `{{ template "<name>" . }}`, resolved against `context/_partials/`. One pool serves both kinds. Expansion happens inside `make bundle`, on the way into the embedded bundle — nothing is written back into `context/`.
...
- Only commands and agents expand. Skills, rules, output styles, and `context/CLAUDE.md` pass through byte-for-byte, so a literal `{{` in their prose is safe.
