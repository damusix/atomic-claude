---
# generated by /refresh-wiki — do not hand-edit; regenerated every refresh
paths: ["atomic/internal/doctor/**", "atomic/internal/validate/**", "atomic/internal/cliusage/**", "atomic/internal/updatedoctor/**", "atomic/internal/profile/**"]
---

Domain: doctor. Integrity checks and static artifact validation.

- Map: docs/wiki/doctor.md
- Contracts: docs/spec/atomic-doctor.md, docs/spec/atomic-validate.md

Consult the map before changing behavior here. Behavior changes stale the pages above. Renames or removals stale mentions beyond them: grep the old name across docs/ before shipping.
