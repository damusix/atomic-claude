...
|------|---------|-----------|
| 1 — primary | Official docs, `context7` MCP, source code itself, RFCs / standards, official changelogs, ast-grep on actual repo, `atomic code callers <symbol>` / `atomic code impact <symbol>` (when a code-intel index is present) | Default. Always try first. |
