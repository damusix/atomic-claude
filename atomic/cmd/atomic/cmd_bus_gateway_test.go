...
// The guide documents `enroll web-api --tls-cert cert.pem`, name first.
// flag.FlagSet.Parse stops at the first positional, so without bus.ParseFlags
// that exact command is a usage error.
func TestGatewayEnrollAction_FlagAfterName_AsTheGuideWritesIt(t *testing.T) {
	home := t.TempDir()
	var out, errOut bytes.Buffer
	if code := gatewayEnrollAction([]string{"web-api", "--tls-cert", "cert.pem"}, home, &out, &errOut); code != int(bus.ExitOK) {
		t.Fatalf("exit code = %d, stderr: %s", code, errOut.String())
	}
	if !strings.Contains(out.String(), "[bus.remotes.web-api]") || !strings.Contains(out.String(), `host = "https://`) {
		t.Fatalf("output = %q, want web-api's block with an https:// host", out.String())
	}
}

// Re-enrolling a name exits "name taken", the same code a join collision
// uses, rather than issuing a second key: revoke works by name, so a
// duplicate could never be revoked alone.
func TestGatewayEnrollAction_ExistingName_ExitNameTaken(t *testing.T) {
	home := t.TempDir()
	var out, errOut bytes.Buffer
	if code := gatewayEnrollAction([]string{"web-api"}, home, &out, &errOut); code != int(bus.ExitOK) {
		t.Fatalf("first enroll exit code = %d, stderr: %s", code, errOut.String())
	}
	out.Reset()
	errOut.Reset()
	if code := gatewayEnrollAction([]string{"web-api"}, home, &out, &errOut); code != int(bus.ExitNameTaken) {
		t.Fatalf("second enroll exit code = %d, want %d; stdout: %q", code, bus.ExitNameTaken, out.String())
	}
	if strings.Contains(out.String(), "key") {
		t.Fatalf("a refused enroll printed a key block: %q", out.String())
	}
}
