...
|------|---------|-----------|
| 1 — primary | Official docs, `context7` MCP, source code itself, RFCs / standards, official changelogs, ast-grep on actual repo | Default. Always try first. |
...
- **Codebase claims** ("function X is called from N places", "we already have a Z pattern") → `ast-grep` first (`sg run -p '<pattern>' -l <lang>`); `grep`/`Grep` for literal strings, log messages, config values.
