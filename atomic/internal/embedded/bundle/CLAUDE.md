...
`atomic doctor [--fix] [--json] [--only <cat[,...]>] [--skip <cat[,...]>] [--stale-days N] [--verbose]` runs eight indexed integrity checks (install, hooks, signals, refs, manifest, followups, memory, binary) against `~/.claude/` and the current project. Exits 0 (all PASS or only WARN/SKIP), 1 (any FAIL), 2 (usage error). `--fix` prompts per item to apply repairs (per axiom 3). Full spec: `docs/spec/atomic-doctor.md`.
