...
- `atomic signals linkify` — render signals path citations (`signals.md` + everything under `signals/`, base = repo root) as file-relative markdown links so the signals graph is Obsidian / md-server / GitHub navigable. Idempotent; not `@-refs`. The inferrer runs this as its final step.
- `atomic wiki linkify --root=<path>` — same for wiki summaries (`repos/**`, base = each summary's `repo:` dir), `concerns/*.md`, and `index.md` (base = realm root). `/refresh-wiki` runs this after stamping. Idempotent; not `@-refs`.
