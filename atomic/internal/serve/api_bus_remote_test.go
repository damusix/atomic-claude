...
// The Bus page's Sessions panel asks /bus/sessions for the open room. With a
// remote "potato" open and a local "potato" alongside it, the panel must list
// the remote room's members, not the local room's.
func TestAPIBus_Sessions_RemoteHost_ListsThatHostsMembers(t *testing.T) {
	baseURL, store := startTestGateway(t)
	rec, err := store.Enroll("prod")
	if err != nil {
		t.Fatalf("Enroll: %v", err)
	...
	postBusJSON(t, srv.URL+"/api/bus/join", map[string]string{"room": "potato"}).Body.Close()
	if _, err := bus.DoRemote(home, "prod", bus.Request{Op: bus.OpJoin, Room: "potato", Name: "remote-only", Session: "sess-remote", Kind: bus.KindAgent}); err != nil {
		t.Fatalf("remote join: %v", err)
	}

	sessionsFor := func(query string) []busSessionInfo {
		resp, err := http.Get(srv.URL + "/api/bus/sessions?" + query)
		if err != nil {
			t.Fatalf("GET sessions: %v", err)
		}
		return decodeBusResponse[busSessionsResponse](t, resp).Sessions
	}

	remote := sessionsFor("room=potato&host=prod")
	if len(remote) != 1 || remote[0].Name != "remote-only" || remote[0].Transcript.Found {
		t.Fatalf("remote sessions = %+v, want only remote-only, with no local transcript", remote)
	}
	for _, s := range sessionsFor("room=potato") {
		if s.Name == "remote-only" {
			t.Fatalf("local sessions list the remote room's member: %+v", s)
		}
	}
}
