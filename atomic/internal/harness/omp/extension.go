...
// session's base: a home-relative or Windows path, a parent escape above the
// base, an absolute path outside it, or no observed base at all. The base is
// the workspace root OMP reports at session start, so a relative path resolves
// against it and an absolute path must already sit inside it. A path that is
// not contained is recorded as uncovered rather than matched against a path it
// does not have.
function rebase(candidate: string, base: string): string | null {
...
// observed. The bound is Atomic's own ceiling on that static index, measured in
// UTF-16 code units — the unit String.length reports, which is the unit the
// plan measures too. The matched and uncovered lines are additions this session
// observed, so they are not part of the ceiling, and nothing bounds the
// assembled block: a second guard over those lines would suppress a session the
// plan reported deliverable. A static index over the bound is not delivered at
// all, because a truncated rule index would misreport which rules are in force.
function sessionBaseline(matched: string[], uncovered: string[]): string | null {
	if (DATA.indexText.length > DATA.bound) return null;
...
export default function atomic(pi: ExtensionApi): void {
	...
	// The base the projected patterns are relative to: the workspace root OMP
	// reports at session start. Until it is observed every operation is
	// uncovered, because no path can be proven contained in an unknown base.
	let base = "";
	let deliveries = 0;
	let operations = 0;
	let denials = 0;
...
// handlerSessionStart observes the tool inventory, the delivered index, and the
// workspace root the runtime matcher rebases operation paths onto.
...
const handlerBeforeAgentStart = `	pi.on("before_agent_start", (event) => {
		...
		if (block === null) {
			emit({ kind: "baseline_suppressed", units: DATA.indexText.length, bound: DATA.bound });
			return;
		}
...
const toolCallMatchBranch = `		const proven = DATA.tools.find((input_) => input_.tool === name);
		...
		const candidate = rebase(text(field(input, proven.path_field)), base);
		if (candidate === null) {
			emit({ kind: "uncovered_operation", tool: name, candidate: text(field(input, proven.path_field)), reason: "the proven field carried no path contained in the session base" });
			return;
		}
		operations += 1;
