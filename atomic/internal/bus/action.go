...
// ParseFlags is parseFlags for `atomic bus` subcommands registered outside
// this package, so every bus verb accepts flags before or after its positionals.
func ParseFlags(fs *flag.FlagSet, args []string) ([]string, error) {
...
// session holds in it (a join may have been renamed), the host it was joined
// on (empty for local), and its halt state.
...
func statusAction(args []string, home string, out io.Writer) int {
	...
	if host != "" {
		...
		if perr == nil {
			...
			if json.Unmarshal(resp.Payload, &payload) == nil {
				...
				status.Rooms = annotateHalted(fetchRooms, host, status.Rooms)
			}
		} else {
	...
	if len(status.Rooms) == 0 {
		...
		for _, r := range status.Rooms {
			...
			fmt.Fprintf(out, "%s\t%s%s%s\n", r.Room, r.Name, on, haltedSuffix(r.Halted, r.HaltReason))
		}
	}
	return int(ExitOK)
}

// annotateHalted fills in Halted/HaltReason by fetching host's `rooms` list and
// matching by name, only for rooms joined on that host: a local "potato" and a
// remote "potato" are different rooms. A second round trip because Client.Do
// consumes its connection and OpPing's payload carries no per-room data.
// Best-effort: any failure leaves rooms exactly as passed in, since status's
// primary job is reporting reachability, not halt state.
func annotateHalted(fetchRooms func() (Response, error), host string, rooms []joinedRoomStatus) []joinedRoomStatus {
	resp, err := fetchRooms()
	if err != nil {
		return rooms
	}
...
func joinedRooms(st *State, session string) []joinedRoomStatus {
	...
	for room, m := range ss.Rooms {
		out = append(out, joinedRoomStatus{Room: room, Name: m.Name, Host: m.Host})
	}
