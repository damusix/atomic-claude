...
// A room joined through --host lives on another machine, so status must say
// which: without the host, "daemon: not running" beside a live remote room
// reads as the room being down.
func TestStatusAction_RemoteRoom_ReportsItsHost(t *testing.T) {
	home := testBusHome(t)
	st, err := Load(home)
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if err := st.Join("sess-1", "checkout", "fe", "participate", KindAgent, "", "", "prod"); err != nil {
		t.Fatalf("Join: %v", err)
	}
	...
	var out bytes.Buffer
	if code := statusAction([]string{"--session", "sess-1", "--json"}, home, &out); code != int(ExitOK) {
		t.Fatalf("exit code = %d, want %d", code, ExitOK)
	}
	var status busStatus
	if err := json.Unmarshal(out.Bytes(), &status); err != nil {
	...
	if len(status.Rooms) != 1 || status.Rooms[0].Host != "prod" {
		t.Fatalf("Rooms = %+v, want checkout reported on host prod", status.Rooms)
	}

	out.Reset()
	if code := statusAction([]string{"--session", "sess-1"}, home, &out); code != int(ExitOK) {
		t.Fatalf("text exit code = %d", code)
	}
	if !strings.Contains(out.String(), "checkout\tfe\ton prod") {
		t.Fatalf("text output = %q, want the room's host on its line", out.String())
	}
}

// Halt state comes from one daemon's room list, matched by name. A local
// "checkout" halted by someone else must not mark this session's remote
// "checkout" halted: they are different rooms that share a name.
func TestStatusAction_LocalHaltDoesNotMarkSameNameRemoteRoom(t *testing.T) {
	home := testBusHome(t)
	mustStartTestDaemon(t, home)

	var discard bytes.Buffer
	if code := joinAction([]string{"checkout", "--as", "local", "--session", "sess-local"}, home, testCwd(t), &discard); code != int(ExitOK) {
		t.Fatalf("local join exit code = %d", code)
	}
	if code := haltAction([]string{"checkout", "--text", "local only"}, home, &discard); code != int(ExitOK) {
		t.Fatalf("halt exit code = %d", code)
	}
	...
	if err := st.Join("sess-remote", "checkout", "fe", "participate", KindAgent, "", "", "prod"); err != nil {
		t.Fatalf("Join: %v", err)
	}
	if err := st.Save(home); err != nil {
		t.Fatalf("Save: %v", err)
	...
	var out bytes.Buffer
	if code := statusAction([]string{"--session", "sess-remote", "--json"}, home, &out); code != int(ExitOK) {
	...
	if len(status.Rooms) != 1 || status.Rooms[0].Halted {
		t.Fatalf("Rooms = %+v, want the remote checkout not halted by the local one", status.Rooms)
	}
}
