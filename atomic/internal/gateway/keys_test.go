...
// A lost key re-enrolled under its old name must not leave the lost key
// valid, and revoke removes every key under a name, so the second enroll is
// refused outright rather than adding a key nobody could revoke alone.
func TestStore_Enroll_ExistingName_Refused(t *testing.T) {
	s := NewStore(filepath.Join(t.TempDir(), "keys.json"))

	first, err := s.Enroll("web-api")
	if err != nil {
		t.Fatalf("Enroll: %v", err)
	}
	if _, err := s.Enroll("web-api"); !errors.Is(err, ErrNameTaken) {
		t.Fatalf("second Enroll(web-api) err = %v, want ErrNameTaken", err)
	}
	if _, ok, err := s.Lookup(first.ID); err != nil || !ok {
		t.Fatalf("the refused enroll disturbed the existing key: ok=%v err=%v", ok, err)
	}
	if _, err := s.Enroll("web-api-2"); err != nil {
		t.Fatalf("Enroll under a new name: %v", err)
	}
}
