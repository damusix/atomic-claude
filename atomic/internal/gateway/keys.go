...
// ErrNameTaken is Enroll's refusal of a name that already has a key. Revoke
// deletes every key under a name, so a second key under one name could never
// be revoked alone.
var ErrNameTaken = errors.New("name already enrolled")
...
// block once. A name that already has a key is refused with ErrNameTaken.
func (s *Store) Enroll(name string) (Record, error) {
	...
	for _, rec := range s.records {
		if rec.Name == name {
			return Record{}, fmt.Errorf("gateway: %q: %w; revoke it first or enroll a different name", name, ErrNameTaken)
		}
	}
