...
// TestRegistryCount verifies exactly 12 entries with stable indices 1–12.
func TestRegistryCount(t *testing.T) {
	cats := doctor.Categories()
	if len(cats) != 12 {
		t.Fatalf("registry len = %d, want 12", len(cats))
	}

	// Indices must be 1..12 with no gaps.
	for i, c := range cats {
		want := i + 1
		if c.Index != want {
...
// TestRunFiltersBySkip verifies Run with Skip=[2,4,6,8] returns indices [1,3,5,7,9,10,11,12].
func TestRunFiltersBySkip(t *testing.T) {
	...
	wantIndices := []int{1, 3, 5, 7, 9, 10, 11, 12}
...
// TestFlagParsingRejectsOutOfRangeIndex verifies --only rejects index 0 and >12.
func TestFlagParsingRejectsOutOfRangeIndex(t *testing.T) {
	_, err := doctor.ParseFlags([]string{"--only", "13"})
	if err == nil {
		t.Fatal("expected error for out-of-range index 13, got nil")
	}
