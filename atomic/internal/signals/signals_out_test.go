...
// has no .claude/project/ directory created.
func TestScanWithOut_WritesToOutDir(t *testing.T) {
	...
	wantPath := filepath.Join(outDir, ".claude", "project", "deterministic-signals.md")
	if _, err := os.Stat(wantPath); err != nil {
		t.Fatalf("substrate not written to outDir: %v (expected %s)", err, wantPath)
	}

	// The scanned repo must NOT have .claude/project/ created.
	repoClaude := filepath.Join(repo, ".claude")
	if _, err := os.Stat(repoClaude); err == nil {
		t.Errorf("scanned repo has .claude/ created — it must not be written when OutDir is set")
	}
}

// TestScanWithOut_DefaultUnchanged asserts that without OutDir the substrate
// still lands in the repo's .claude/project/ path (default behavior unchanged).
func TestScanWithOut_DefaultUnchanged(t *testing.T) {
	...
	wantPath := filepath.Join(repo, ".claude", "project", "deterministic-signals.md")
	if _, err := os.Stat(wantPath); err != nil {
...
func TestScanWithOut_ContentMatchesDefault(t *testing.T) {
	...
	if err := signals.ScanWithOptions(repoA, &signals.Options{Clock: fixedClockFn}); err != nil {
		t.Fatalf("default scan: %v", err)
	}
	defaultPath := filepath.Join(repoA, ".claude", "project", "deterministic-signals.md")
	...
	// --out scan on repoB.
	outDir := t.TempDir()
	if err := signals.ScanWithOptions(repoB, &signals.Options{Clock: fixedClockFn, OutDir: outDir}); err != nil {
		t.Fatalf("out scan: %v", err)
	}
	outPath := filepath.Join(outDir, ".claude", "project", "deterministic-signals.md")
