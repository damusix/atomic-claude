...
- **Q3. Worktree multiplication.** Each worktree (`.claude/worktrees/<branch>/`) has its own `.claude/project/`. Domain files multiply per worktree and may drift between branches. Probably fine (each worktree reflects its branch's truth) but the doctor `signals` check must not cross-compare worktrees.
