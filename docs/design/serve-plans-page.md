...
A version therefore holds a *set* of checkouts, which decides both how it is labelled and how it is found. It is labelled by the merged checkout when one is in the set, and by the most recently modified otherwise — one representative name, so the picker stays scannable. But it is *searchable by every name in the set*: typing `deslop` finds the version above even though the entry reads `main`. Type-ahead over the full set costs nothing and closes the gap between "the branch I am working on" and "the label this version happens to wear".
