...
- **→ doctor**: `claudeinstall.Diff` (this domain) is called by `checks_install.go` (doctor category 1, `install` check) to detect drift between the embedded bundle and the on-disk install; `Diff` now applies `[agents]` tier overrides before comparing, so a tier-customized install ([`atomic/internal/config`](../../atomic/internal/config) `[agents]` block) no longer produces a false-positive WARN.
