...
<scan-sha>3b6a97f6b7b0dfa21c45dd44ab1f4316b55e328c</scan-sha>
...
A Claude Code, OMP, and Codex configuration — a global contract, commands, agents, skills, rules, and an output style — plus the [`atomic`](../../atomic) Go CLI that renders, installs, and maintains it. The markdown artifacts are the product; the binary is how they ship, project into each harness natively, and stay healthy.

Everything installable is committed source under [`context/`](../../context). One step turns it into a running install: `make bundle` expands [`context/`](../../context) (substituting any `{{ template "<name>" . }}` reference to [`context/_partials/`](../../context/_partials) along the way) straight into the Go binary's embedded filesystem. The harness adapters then render that canonical corpus into native bytes per target — Claude's global [`CLAUDE.md`](../../CLAUDE.md), an `AGENTS.md` plus thin [`CLAUDE.md`](../../CLAUDE.md) loader pair per scope, an OMP profile package, or a Codex plugin — and `atomic install` / `atomic harness enroll` writes them and records ownership in `~/.atomic/install/ledger.json`.

```mermaid
flowchart LR
    CTX["context/<br/>agents, commands, skills,<br/>rules, output-styles, AGENTS.md"] -->|make bundle| EMB["atomic/internal/embedded/<br/>go:embed"]
    EMB --> BIN["atomic binary"]
    BIN -->|atomic install / harness enroll| ADAPT["harness adapters<br/>Claude · OMP · Codex"]
    ADAPT -->|native projection| H["~/.claude/ · OMP profile · Codex plugin"]
```

Edit [`context/`](../../context) directly; nothing downstream of it is ever hand-edited. [`atomic/internal/embedded/`](../../atomic/internal/embedded) is a gitignored build artifact, regenerated by `make bundle` and never committed. The binary carries more than the installer: `doctor`, `wiki`, `code`, `serve`, `bus`, `repl`, and `retro` are the other verb families, each mapped to a domain below.
...
|--------|-----------|--------------|--------|
| signals | [`atomic/internal/signals/`](../../atomic/internal/signals) | Scan, infer, and wire the project context a session loads each turn. | [`docs/wiki/signals.md`](signals.md) |
| bundle | [`atomic/internal/bundlemirror/`](../../atomic/internal/bundlemirror) | Expand [`context/`](../../context) into the embedded binary and map it to Claude-native targets. | [`docs/wiki/bundle.md`](bundle.md) |
| harness | [`atomic/internal/harness/`](../../atomic/internal/harness) | Enroll harness targets, project native artifacts, and track ownership in a ledger. | [`docs/wiki/harness.md`](harness.md) |
...
- **No web framework.** SQLite is used only by the code-intel engine, at `<state-root>/.atomic-index/atomic.db`.
...
| `github.com/spf13/cobra` + `pflag` | the [`atomic`](../../atomic) command tree and flag parsing |
| `github.com/yuin/goldmark` | markdown parsing for `atomic validate` and the serve renderer |
| `github.com/alecthomas/chroma/v2` | syntax highlighting in the serve markdown renderer |
| `github.com/tetratelabs/wazero` | WASM runtime for tree-sitter (indirect, via `tsbinding`) |
| `github.com/modelcontextprotocol/go-sdk` | MCP server |
| `github.com/bmatcuk/doublestar/v4` | glob matching for repo-scoped code-intel ignore config and rule `paths:` globs |
| `modernc.org/sqlite` | the code-intel SQLite index |
