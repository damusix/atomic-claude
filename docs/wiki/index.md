...
<scan-sha>e7f83d7927a868031e6985fc4b836bac1e4b49c4</scan-sha>
...
Only the leftmost column is written by hand. Every stage after it is generated from the one before, which is why an edit to a rendered file disappears on the next build rather than failing.

```mermaid
flowchart LR
    T["templates/<br/>commands, agents, shared"] -->|make render| R["commands/<br/>agents/"]
    R --> B["make bundle<br/>go:embed"]
    S["skills/, rules/,<br/>output-styles/, CLAUDE.md"] --> B
    B --> BIN["atomic binary"]
    BIN -->|atomic claude install| H["~/.claude/"]
```

Edit [`templates/`](../../templates) and the bundled sources; never edit [`commands/`](../../commands) or [`agents/`](../../agents) directly — `make render` overwrites them. The binary carries more than the installer: `doctor`, `wiki`, `code`, `serve`, `bus`, and `repl` are the other verb families, each mapped to a domain below.
...
| workflow | [`commands/`](../../commands) | Plan, implement, review, ship — and the subagent loop that drives it. | [`docs/wiki/workflow.md`](workflow.md) |
...
| docs-meta | [`skills/atomic-writing/`](../../skills/atomic-writing) | Writing voice and diff-driven documentation routing. | [`docs/wiki/docs-meta.md`](docs-meta.md) |
...
- **`//go:embed all:bundle`** in [`atomic/internal/embedded/bundle.go`](../../atomic/internal/embedded/bundle.go). The `all:` prefix is required: the plain `bundle` glob skips underscore-prefixed paths, which would silently drop [`commands/_templates/`](../../commands/_templates).
...
Two CI drift gates run in this order. Render writes [`commands/`](../../commands) and [`agents/`](../../agents) and bundle reads them, so reversing the two embeds stale output:

```
make render -> git diff --exit-code    stale commands/ or agents/ fails
make bundle -> git diff --exit-code    stale manifest.go fails
```
...
- **CI**: GitHub Actions ([`.github/workflows/`](../../.github/workflows)). Four workflows — `ci.yml` (test, vet, fmt, drift gates), `docs.yml` (VitePress build and deploy), `release-please.yml` (changelog), `release.yml` (goreleaser on tag).
...
- **Pre-commit hook** ([`.githooks/pre-commit`](../../.githooks/pre-commit)): four stages in order — render, bundle, followups `INDEX.md`, and a `serve/frontend/dist` rebuild — each firing only when a staged path touches its own inputs. Install with `make hooks`.
...
|----------|-----|-------|---|
| Go | 188750 | 518 | 59% |
| Markdown | 61962 | 385 | 19% |
| TypeScript | 37511 | 239 | 11% |
