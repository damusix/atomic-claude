...
### The review-gate hook pair

`atomic hooks install` registers two more Claude Code hooks alongside session-start: `PostToolUse` (matcher `Edit|Write|MultiEdit|NotebookEdit|Agent`, command `atomic hooks post-tool-use`) and `Stop` (command `atomic hooks stop`). Together they backstop the ad-hoc review gate for edits a main agent makes outside any command loop, since the loop commands (`/subagent-implementation`, `/autopilot`, and the rest) already gate themselves.

`PostToolUse` appends one JSON line per event to `<os temp dir>/atomic-hooks/<session_id>.jsonl`: an `Edit`/`Write`/`MultiEdit`/`NotebookEdit` call records its `file_path`, and an `Agent` call dispatching `atomic-reviewer` or `atomic-auditor` records a fingerprint of the gated set at that moment. A tool call carrying `agent_type` (made inside a subagent) is skipped, so only the main agent's own edits are recorded. `Stop` folds that log (`foldState`) into a deduped touched-path set and the last recorded review fingerprint, recomputes the gated set (`gatedPaths`: touched paths resolved under the repository root via a symlink-resolved `rev-parse --show-toplevel`, filtered to what a path-scoped `status --porcelain -z` reports dirty, minus anything `isDocsPath` calls documentation), hashes it (`fingerprint`: `diff HEAD` over the set plus, per untracked path, its path and file bytes), and blocks, a non-empty message the CLI maps to exit 2, only when that hash differs from the last recorded review. `stop_hook_active` caps a block at once per turn. Every internal failure (bad JSON, an unsafe `session_id`, a failing subprocess call) is a silent no-op: both hooks fail open rather than block a turn they cannot reason about.

The full block-decision flow, the doc-path exemption list, and the blind spot around edits made through a shell rather than an edit tool are diagrammed in [`docs/reference/hooks.md`](../reference/hooks.md); this page covers where the mechanism lives, not the mechanism's own contract.
...
| Path | Role |
|------|------|
| [`atomic/internal/hooks/hooks.go`](../../atomic/internal/hooks/hooks.go), `hooks_hujson.go` | Session-start hook payload, plus install/uninstall of the three-entry (`SessionStart`/`PostToolUse`/`Stop`) settings.json registration and drift detection. `hujson` parses settings leniently. |
| [`atomic/internal/hooks/reviewgate.go`](../../atomic/internal/hooks/reviewgate.go), `reviewgate_test.go` | `PostToolUse`/`Stop` review-gate hook logic: append-only JSONL event log per session, state fold, gated-set computation (repo-relative, dirty, non-docs), diff fingerprinting, block decision. |
...
| [`docs/spec/review-gate-hook.md`](../spec/review-gate-hook.md) | The `PostToolUse`/`Stop` review-gate hook pair: state schema, the gated-set computation, the fingerprint, and the three-entry `atomic hooks install` contract. |
| [`docs/design/review-gate-hook.md`](../design/review-gate-hook.md) | Why the gate is a hook backstop rather than the sole enforcement mechanism, and why it fails open. |
| [`docs/reference/hooks.md`](../reference/hooks.md) | User-facing contract for all three Claude Code hooks: what each does, the review-gate flow, opt-out, doctor coverage. |
...
**All three hooks are inline commands, not scripts.** `Install` registers the literal strings `atomic hooks session-start`, `atomic hooks post-tool-use`, and `atomic hooks stop` in `settings.json`; nothing is written to disk. `IsInstalled(scopeRoot)` returns `(installed, drifted, err)`: `installed` tracks the session-start hook's own presence (inline or legacy wrapper-script), and `drifted` is true when the legacy wrapper-script registration is still present, or when the inline session-start hook is present but the `PostToolUse` or `Stop` registration (`gateHookMissing`) is absent. Three best-effort ride-alongs run inside the session-start hook and are individually silent on failure: profile refresh, wiki staleness (30-day threshold), and an `atomic where` orientation nudge that stays quiet in the plain no-wiki, no-realm case.
...
| Direction | Contract |
|-----------|----------|
| → doctor | `checks_hooks.go` (category 2) calls `hooks.IsInstalled` and WARNs when any of the three hooks is missing, when the legacy wrapper script is still registered, or when `drifted` reports the `PostToolUse`/`Stop` review-gate pair absent while session-start is present; `--fix` re-runs `hooks.Install`. |
...
| → workflow | The `PostToolUse`/`Stop` review-gate hook pair backstops the ad-hoc review gate for main-agent edits made outside any command loop; the loop commands' own gates (`atomic-verify`, and the ship verbs, which dispatch `atomic-auditor`) are a workflow-domain fact, not restated here. |
