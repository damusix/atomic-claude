...
Extraction never creates a cross-file edge, which is why extraction and resolution can run at different times and on different file sets.

```mermaid
flowchart TD
    W["walk project files<br/>filterIgnored"] --> E{"extension"}
    E -->|"tree-sitter language"| TS["TreeSitterExtractor<br/>wazero pool"]
...
Kind strings persist in SQLite, so each slice's membership is asserted in a test, and the slice, not the comment beside it, is the truth:

| Slice | Pinned by |
|-------|-----------|
| `AllNodeKinds` | `TestNodeKindCount` |
| `AllEdgeKinds` | `TestEdgeKindCount` |
| `AllLanguages` (includes `unknown`) | `TestLanguageCount` |

`stage`, `stream`, `task`, `model`, `file_format`, `macro`, and `script` are SQL-dialect only.

**`ReferenceKindSQLString` and `ReferenceKindSQLFragment` are `EdgeKind` constants that are not edge kinds.** Both are valid only in `UnresolvedReference.ReferenceKind`, and are deliberately excluded from `AllEdgeKinds`. Edges built from them always carry `Kind: references`.
...
**Embedded SQL spans Shape 1 and Shape 2 grammars across the embedded-literal host languages.** Go uses a hand-written scanner, Python and TypeScript/TSX use dedicated tree-sitter harvesters, and every other language in `embeddedLiteralConfigs` goes through `HarvestEmbeddedLiterals`. Shape 1 grammars expose a content child; Shape 2 grammars require taking the node's own text, splicing `"?"` over interpolation byte ranges, and stripping the delimiter alphabet. The host extension set derives from `extToLanguage` plus the config table rather than a second hand-maintained list. `IsSQLLiteral` recognizes SQL structure only, with no stopword list; concatenated and multi-fragment queries are accepted false negatives.
...
Only a resolved match deletes a ref. An unmatched one is kept on purpose, because a later index may add the node it wants.
...
extraction/     TreeSitterExtractor, wazero pool, string-literal harvesters
  languages/    per-language LanguageExtractor configs + registry
...
resolution/     reference -> edge pipeline, import resolver, name matcher
  frameworks/   web-route resolvers
...
| [`atomic/internal/codeintel/cli/code.go`](../../atomic/internal/codeintel/cli/code.go) | `RunCode` dispatches `index sync status search callers callees impact node files affected explore mcp comments`. `mcp` (a proxy to a daemon, not a query) and `comments` (a `git diff` line scan that never opens the index) are both handled before the engine is built, so they skip it entirely. `EnsureGitignore` appends the index dir to [`.gitignore`](../../.gitignore). |
| [`atomic/internal/codeintel/cli/realm.go`](../../atomic/internal/codeintel/cli/realm.go) | `RunCodeWithRealm` resolves scope before `repoctx.Resolve`, because a realm root need not be a git repo. `mcp --daemon` and `comments` both bypass realm resolution and dispatch straight to `RunCode` — a spawned daemon carries explicit `--source`/`--db` and would be killed by the realm-verb-reject gate otherwise, and `comments` resolves the plain git root (`repoctx.Resolve`, falling back to cwd) with no fan-out, since a diff-scan has no per-member concept. Repo scope and no-index scope forward to `RunCode` byte-for-byte. |
| [`atomic/internal/codeintel/mcp/server.go`](../../atomic/internal/codeintel/mcp/server.go) | MCP server. `explore`, `search`, `node` always registered; `callers`, `callees`, `impact`, `status`, and `files` gated on `fileCount >= 500`. |
...
| [`atomic/internal/codeintel/cli/comments.go`](../../atomic/internal/codeintel/cli/comments.go) | `runComments`: a per-file `git diff -U0` line scan for added comments, classified by a per-extension `commentSyntax` table (line and block markers). Reports a run of consecutive added comment lines as one entry, flags one `Over: true` when its line count exceeds `[comments] max_lines` (config), and untracked files (a bare `--diff` with no `..`) are scanned in full via `git ls-files --others --exclude-standard`. Exit 1 when any entry is over the limit. |

### Extraction

| Path | Role |
|------|------|
| [`atomic/internal/codeintel/extraction/extractor.go`](../../atomic/internal/codeintel/extraction/extractor.go) | `TreeSitterExtractor`: a file through the grammar to `ExtractionResult{Nodes, Edges, UnresolvedReferences, Errors}`. `LanguageExtractor.FunctionScopeTypes` drives the local-variable suppression gate. |
| [`atomic/internal/codeintel/extraction/helpers.go`](../../atomic/internal/codeintel/extraction/helpers.go) | `GenerateNodeID`: `kind + ":" + hex(sha256("filePath:kind:name:line"))[:32]`. File nodes and package nodes break the formula: file nodes are `"file:" + filePath`, package nodes are `"package:npm/" + name`. |
| [`atomic/internal/codeintel/extraction/languages/registry.go`](../../atomic/internal/codeintel/extraction/languages/registry.go) | Maps `types.Language` values to grammar configs. `JSX` and `TSX` share the same grammar. |
...
| [`atomic/internal/codeintel/extraction/embedded_literals.go`](../../atomic/internal/codeintel/extraction/embedded_literals.go) | `HarvestEmbeddedLiterals`: a generic harvester covering every embedded-SQL host language without a dedicated harvester (Go, Python, TypeScript/TSX are the exceptions), driven by an `EmbeddedLiteralConfig`. |
...
| [`atomic/internal/codeintel/indexer/sql_fragment_harvest.go`](../../atomic/internal/codeintel/indexer/sql_fragment_harvest.go) | The fragment tier for builder args like `where("title LIKE ?")`. Gate: at most 160 chars, at least one identifier token, at least one discriminator. Tokenizes past a stoplist into `sql_fragment` refs. |
| [`atomic/internal/codeintel/indexer/embedded_literals_config.go`](../../atomic/internal/codeintel/indexer/embedded_literals_config.go) | Per-language grammar node kinds for the generic-harvester host languages (everything `embeddedLiteralConfigs` covers). Probed from live grammars. |
...
| Path | Role |
|------|------|
| [`atomic/internal/codeintel/resolution/pipeline.go`](../../atomic/internal/codeintel/resolution/pipeline.go) | `ResolveAndPersistBatched`. `resolveOne` runs these steps in order: external skip, pre-filter, JVM FQN fast path, framework resolve, import resolve, name match, highest-confidence pick. Import refs skip name match; SQL string and fragment refs skip the loop entirely. |
| [`atomic/internal/codeintel/resolution/resolver.go`](../../atomic/internal/codeintel/resolution/resolver.go) | `ResolveImport` classifies an import as internal, external, or unresolved, and derives `PackageName` for JS-family externals. |
...
**The kind-count comments in the source are wrong.** The section header comments in [`atomic/internal/codeintel/types/types.go`](../../atomic/internal/codeintel/types/types.go) state node-type and edge-type totals that the slices beneath them have since outgrown, and that `TestNodeKindCount` and `TestEdgeKindCount` contradict. Trust the slices and the tests, not the comments.
...
- **config** owns the paths. `engine.go`, `mcp/daemon.go`, `realm/resolver.go`, and `cli/code.go` all resolve the index location through `config.IndexDir` / `config.IndexDBPath` / `config.RepoConfigPath` in [`atomic/internal/config/harness.go`](../../atomic/internal/config/harness.go), so a `harness.dir` change moves the index everywhere at once. Repo-scoped ignore globs come from `config.LoadRepoConfig` + `config.NewIgnoreMatcher`, called by `engine.ensureIndexer`. `cli/comments.go` reads the same `config.LoadRepoConfig` and resolves its line-count threshold through `config.ResolveMaxLines(cfg.Comments)` — the `[comments] max_lines` key, defaulting to 2 and warning (never failing) below 1.
