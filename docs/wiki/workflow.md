...
| [`context/agents/atomic-investigator.md`](../../context/agents/atomic-investigator.md) | Read-only locator. Returns a `file:line — what` table, no prose. Dispatched by default for exploration, since its searches stay out of the caller's context. |
| [`context/agents/atomic-strategist.md`](../../context/agents/atomic-strategist.md) | Read-only "is this the right approach?" reasoning. Dispatched when the loop is stuck (same blocking signal, 2 consecutive `CHANGES_REQUESTED` rounds) — across every loop this engine drives, including `/implement`'s per-checkpoint stuck check. Never a finalize gate; its output is never parsed for a `VERDICT:` line. |
...
- The YAGNI ladder in [`context/_partials/agent-yagni.md`](../../context/_partials/agent-yagni.md) is kept verbatim identical to the ladder in [`context/AGENTS.md`](../../context/AGENTS.md)'s Principles block. Nothing enforces the match, so that duplication is manual.
