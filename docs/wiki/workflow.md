...
**Readability is a defect class, and it skips the ledger in every loop, not just `/autopilot`.** The shared `agent-readability.md` partial, composed into `atomic-implementer`, `atomic-reviewer`, and `atomic-auditor`, floors three findings at 🟡 risk and makes any one of them enough to drive `CHANGES_REQUESTED`:
...
Every EXIT fires before a line of code is written; only a HOLD on all four questions lets `/implement` proceed, and the command names the failing signal in one line before printing the handoff.
...
### Ad-hoc direct edits: auditor-gated without a loop

Editing code directly in the main agent with no command around it at all, and no `atomic-implementer` dispatch, is the one implementation path with no loop of its own — distinct from `/implement`, which also skips the implementer subagent but keeps the checkpoint loop above. Ad-hoc edits have nothing to gate each round, so `atomic-auditor`, dispatched in `diff: working` mode, is dispatched at two exits instead of one — the role `atomic-reviewer` held here until `atomic-auditor` gained the mode.

```mermaid
flowchart LR
    E["Main agent edits code<br/>(no atomic-implementer)"] --> V{"atomic-verify:<br/>claiming ready?"}
    V -->|yes| R1["atomic-auditor<br/>(diff: working)"]
    R1 -->|PASS, 0 unaddressed 🔴| Ready["claim ready"]
    R1 -->|🔴 findings| E
    E --> S["git add"]
    S --> G{"review-gate:<br/>loop-produced,<br/>already audited,<br/>or docs-only?"}
    G -->|no| R2["atomic-auditor<br/>(diff: working)"]
    G -->|yes| Commit["commit"]
    R2 -->|PASS| Commit
    R2 -->|🔴 bug| E
```

[`context/skills/atomic-verify/SKILL.md`](../../context/skills/atomic-verify) blocks a "ready" claim on code written this way — outside `/implement`, `/subagent-implementation`, `/quick-fix`, `/autopilot`, or `/subagent-diagnose` — until `atomic-auditor`, dispatched with `diff: working` and `intent:`, reports `VERDICT: PASS` with every 🔴 and 🟡 finding addressed. [`context/_partials/review-gate.md`](../../context/_partials/review-gate.md), composed into `commit-flow` and `squash-flow-steps` as the step before `doc-impact`, skips the same five loop-produced sources, skips a change `atomic-verify`'s gate already audited this session, and otherwise dispatches `atomic-auditor` with `diff: working` and `intent:` on the staged diff before the commit lands, acting on the verdict: 🔴 blocks and must be fixed and re-staged, 🟡 needs a fix or a one-line justification, 🔵 is mentioned, never blocking. Both exits state `audit: <verdict> (atomic-auditor on <model>)` before the claim or the commit, since the auditor carries no `model:` pin and runs on the session's model.

An `atomic hooks stop` Claude Code hook backstops both exits: it blocks the main agent's turn from ending while an edit sits uncommitted and unreviewed by `atomic-auditor` or `atomic-reviewer`. Hook registration and its per-session fingerprint tracking are config-domain state — see [`docs/wiki/config.md`](config.md).
...
**The auditor is dispatched exactly once per task, in loop mode.** A `CHANGES_REQUESTED` verdict earns one more implementer and reviewer iteration, then the run continues regardless of what a second audit would say. Re-auditing turns finalize into an unbounded loop, which never terminates under `/autopilot`. `/implement`'s final gate is the same single `atomic-auditor` dispatch, not a separate choice among agents. In `diff: working` mode — `atomic-verify`'s readiness check and the ship verbs' review gate, on code written ad-hoc — there is no loop to bound: each gate event is its own dispatch, and a fix made after `CHANGES_REQUESTED` re-opens the gate and is audited again.

**The review gate runs once per change, not once per step.** [`context/_partials/review-gate.md`](../../context/_partials/review-gate.md) skips when the staged work already came from a loop or checkpointed command that dispatches `atomic-reviewer` per iteration or per checkpoint (`/implement`, `/subagent-implementation`, `/quick-fix`, `/autopilot`, `/subagent-diagnose`), when `atomic-verify`'s gate already dispatched `atomic-auditor` on this same change in this session, when this same gate already reviewed the change earlier in the flow (a squash reviewing what commit-time already reviewed), or when every staged path is documentation, by the same test the signals gate uses. Without the guard, a squash of already-reviewed commits would re-review the identical diff. The once-per-change rule bounds this gate's own repeats within one flow; it does not cap how many times `atomic-auditor` runs across separate gate events (readiness check, then commit-time), each of which is its own dispatch.
...
- `/subagent-implementation`, `/quick-fix`, and `/implement` are co-consumers of the same `atomic prompt reviewer` brief and the same scratchpad trio contract (`BRIEF.md` / `STATE.md` / `FOLLOWUPS.md`) — a shape change to any of the three needs updating all three command files.
