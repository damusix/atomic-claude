...
`checks_config.go` — imports [`atomic/internal/config`](../../atomic/internal/config) directly. Validates config file structure, known key set (now including `harness.dir` and `repl.idle_timeout`), and value constraints — including `[claude.agents.<name>]`: an invalid `effort` value FAILs via `config.Validate`; `model` is lenient and never fails the check (`checks_config_test.go` asserts effort-fail / lenient-model-pass; the prior invalid-tier test was retired alongside the tier allowlist). A present-but-invalid `[repl] idle_timeout` (unparseable by `time.ParseDuration`, or zero/negative) also FAILs via `config.Validate`'s call to `config.ValidateIdleTimeout` (config domain); an absent value is not an error.
