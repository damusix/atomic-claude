...
`checks_config.go` — imports [`atomic/internal/config`](../../atomic/internal/config) directly. Validates config file structure, known key set (now including `harness.dir`), and value constraints — including `[claude.agents.<name>]`: an invalid `effort` value FAILs via `config.Validate`; `model` is lenient and never fails the check (`checks_config_test.go` asserts effort-fail / lenient-model-pass; the prior invalid-tier test was retired alongside the tier allowlist).
