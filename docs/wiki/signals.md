...
| [`docs/wiki/index.md`](index.md), the **router** | a model | every turn, via an `@`-ref in the project steering file (`AGENTS.md`, a local override, or the [`CLAUDE.md`](../../CLAUDE.md) loader beside it) |
| `docs/wiki/<domain>.md` | a model | on demand, when a task reaches that domain |
| `<state-root>/rules/wiki/<domain>.md`, a **pointer card** ([`.claude`](../../.claude) by default) | a model | on demand, whenever a session touches a file matching that domain's `paths:` glob |

Only the router is `@-ref`'d, so the router alone is what every turn pays for. It points; detail lives one hop away in a domain file — this page is one. `scan.md` is the raw substrate underneath both: committed, read on demand by the inferrer, never loaded into a session. The pointer card is the fourth artifact: it never carries wiki content, only a link to the domain page and the domain's related docs, delivered by the harness's native `paths:`-scoped rule injection rather than an `@`-ref.
...
flowchart TD
    ...
    E --> F["@-ref wired into<br/>the project steering file"]
...
After [`docs/wiki/index.md`](index.md) is assembled (Step 7 of the repo pipeline), the inferrer emits one `<state-root>/rules/wiki/<domain>.md` card per domain (Step 7b) — [`.claude`](../../.claude) under the default state root, this repo's [`.claude/rules/wiki/`](../../.claude/rules/wiki) — before it checks the `@-ref` wiring (Step 8). Each card is a wiki-producer rule record: [`atomic/internal/rules/`](../../atomic/internal/rules) parses it as `rules/wiki/<domain>.md`, class `wiki-pointer`, identity `wiki:<project-key>:<domain>`. Which domains get a rewritten card, and which cards get deleted, follows a scope decision made once per run:

```mermaid
flowchart TD
    S["Step 7b starts"] --> Q1{"the rules/wiki card dir<br/>absent or holds no cards?"}
...
| Path | Role |
|------|------|
| [`context/agents/atomic-wiki-inferrer.md`](../../context/agents/atomic-wiki-inferrer.md) | The orchestrator. Detects repo vs realm scope, loads the matching pipeline reference, sub-dispatches `atomic-wiki-writer` per domain and `atomic-reviewer` per domain file, then assembles the router and emits the pointer cards. Writes nothing outside the active wiki root, `<state-root>/rules/wiki/` ([`.claude/rules/wiki/`](../../.claude/rules/wiki) by default), an ignore file's rules/wiki negation append, and the single `@-ref` target. |
| [`context/agents/atomic-wiki-writer.md`](../../context/agents/atomic-wiki-writer.md) | Authors one domain page from source. Carries the `atomic-writing` skill in frontmatter rather than being asked to invoke it, and does not fan out. |
...
| [`context/skills/atomic-wiki/references/repo.md`](../../context/skills/atomic-wiki/references/repo.md) | The repo-scope pipeline the inferrer executes (Steps 1-9, including 7b, the pointer-card step). The inferrer resolves `references/repo.md` relative to the `atomic-wiki` skill that ships it, not from this repo. |
| [`context/_partials/signals-gate.md`](../../context/_partials/signals-gate.md) | The `signals-gate` partial composed into ship verbs. Owns the docs-only guard, the `atomic signals stale` exit-code routing, the `<steering>` block passed to the silent dispatch, and staging `<state-root>/rules/wiki/` ([`.claude/rules/wiki/`](../../.claude/rules/wiki) by default) plus any modified ignore file after a silent refresh. |
...
| [`atomic/internal/rules/`](../../atomic/internal/rules) | Parses each emitted pointer card — `ParseWiki` / `LoadWiki` — into a wiki-producer rule record (class `wiki-pointer`, identity `wiki:<project-key>:<domain>`), then binds and projects it onto a harness's native scope. Two producers share this package and cannot collide on ID. |
...
| [`docs/spec/signals-wiki-linkify.md`](../spec/signals-wiki-linkify.md) | Linkify contract: disk resolution as the filter, the skip-set, gitignore layer, idempotence, the angle-bracket destination form, fenced blocks untouched. |
...
| [`docs/spec/signals-project-detection.md`](../spec/signals-project-detection.md) | Framework and dependency annotations on manifest bullets. Unimplemented: `manifests.go` emits no detection annotations. The spec also predates the current layout, naming `deterministic-signals.md` and [`.claude/project/`](../../.claude/project); the scan's exclude and generated globs are resolved by `config.ScanGlobs` from `[scan]` in [`.claude/atomic.toml`](../../.claude/atomic.toml), falling back to a legacy `.signalsignore`. |
...
**Write backtick paths, never markdown links and never `@-refs`.** `atomic signals linkify` renders resolvable tokens into relative links afterward, skipping `scan.md`, the steering loader pair (`AGENTS.md` and its [`CLAUDE.md`](../../CLAUDE.md) loader), and fenced code blocks, and rewriting a file only when the content changed. A destination holding a space, tab, or parenthesis is written in angle brackets, `` [`path`](<rel path>) ``, since a bare one would not parse as a link; a path that itself contains `<` or `>` is left as plain text. A token that does not resolve on disk is left as plain text, which is how `` `git status` `` and `` `atomic signals scan` `` survive. A token built only from `.` and `/` is left alone too — it would otherwise resolve to the repo root and link there.
