...
- **→ workflow**: the `/subagent-implementation` finalize step and `/autopilot` pre-ship step dispatch `atomic-signals-inferrer` in silent mode with `changed_range: <loop-base>..HEAD`, committed as `chore(signals): refresh after <topic>`. Ship verbs dispatch the inferrer as an ad-hoc fallback only when `atomic signals stale` exits 1 and the staged set is not docs-only (docs-only commits skip before the staleness check). If the agent's interface or output contract changes, the finalize templates and signals-gate partial (bundle domain) must be updated.
