...
- [ ] **SC1** — Motion policy holds: a fresh mount (no cached positions) runs
      ...
      dragging a node reheats the live simulation with bounded energy — no
      pinning: springs carry direct neighbors along, repulsion resolves
      overlaps (dropping a node onto another pushes it aside), and force
      damping keeps distant nodes near-still; releasing the drag cools the
      simulation back to a pause and writes a full position snapshot to the
      cache. The cache is
...
### 2026-07-05 — Drag is a live reheat; pinning removed

**What changed:** SC1's drag clause and CP3 no longer pin the non-dragged graph. A drag reheats the live simulation with bounded energy: springs move direct neighbors, repulsion resolves overlaps, damping keeps distant nodes near-still. Release still cools to a pause and snapshots the cache.

**Why:** Live user testing — with pinning, dropping a node onto another left them overlapped and the map felt inert. The original stays-put rule guarded against cola's constraint-solver lurch, which does not exist under cosmos's damped physics; the owner reversed the decision after feeling both behaviors.

**Superseded:** pin-the-non-neighborhood drag containment (`setPinnedPoints` during drag).
