...
- [ ] **SC1** — Motion policy holds: a fresh mount (no cached positions) runs
      ...
      fingerprint; while paused and the viewport is idle, the view performs
      zero per-frame work (no position readback, no DOM label writes);
      ...
      a pause and writes the dragged node's new position to the cache
      (position saves happen on drag release only, never on open). Continuous
