...
- [ ] Every wire message carries a protocol version. A harness spawned by an older binary that
      ...
      client) fails loud — the client exits with the protocol-mismatch code and the message
      names `repl stop` then `start` as the fix — and never attempts to parse a response
      against a version it doesn't recognize. The code is distinct from session-dead because
      the remedies diverge: a mismatched harness is still alive, so a bare `start` reports
      already-running instead of fixing anything.
...
- [ ] Exit codes are fixed, distinct, and documented — callers route on the code, never on
  ...
  | 6 | interpreter/environment unavailable (`--lang`'s interpreter not on PATH, or `--bin`
      does not resolve) |
  | 7 | protocol version mismatch (a live harness speaks a different `v` than the client —
      remedy is `repl stop` then `start`, distinct from 5 whose remedy is `start` alone) |
...
- [ ] An invalid `[repl] idle_timeout` (unparseable, zero, or negative) present at either
      scope emits one stderr line at `start` naming that config file and the value, then
      resolution continues to the next tier; the other verbs never emit it and no exit code
      changes.
...
atomic/internal/repl/action.go
  ReplAction — verb dispatch
    ...
    resolveIdleTimeout — repo [repl] idle_timeout, else user [repl] idle_timeout, else 1h;
      returns per-scope invalid-value warnings for startAction to print on stderr
...
docs/reference/repl.md
  ...
  Exit codes — the eight-code table
...
6. CLI materializes the embedded harness script for --lang and spawns it detached (Setsid, nil
   ...
   an invalid value at either scope prints one stderr warning naming the file + value, then
...
### 2026-08-08 — distinct exit code for protocol mismatch

**What changed:** Exit code 7 (protocol version mismatch) added to the table; the
version-mismatch success criterion now requires it; the `ExitCode` constant set gains
`ProtocolMismatch`; CP1's lock test and CP3's per-code scenarios cover eight codes.

**Why:** CP3 review — a mismatched harness is still alive, so it shares no remedy with
session-dead (5): a bare `start` probes live and reports already-running forever. An agent
routing on codes (the table's own stated purpose) cannot distinguish "run `start`" from
"run `stop` then `start`" without parsing prose.

**Superseded:** Protocol mismatch shared exit 5 with session-dead, distinguishable only by
message text.

### 2026-08-08 — start-time warning for invalid idle_timeout

**What changed:** New success criterion: an invalid `[repl] idle_timeout` at either scope
emits one stderr line at `start` naming the config file and the value, then resolution
continues to the next tier; other verbs stay silent; exit codes unchanged.

**Why:** CP4 review — resolution fell through silently and only `atomic doctor` flagged the
bad value, against the graphignore use-time-warning precedent and the fail-loud principle. A
future reader could otherwise remove the shipped warning as unspecified.

**Superseded:** Invalid values fell through with doctor as the only flagging surface.
...
**Squashed to a single branch commit — 2026-08-08.** Per-iteration SHAs below are historical (unreachable from any branch).

Built across 8 iterations of /subagent-implementation (6 checkpoints + 2 fix rounds). Commits (chronological):

- `93ac0eb` — CP-1 embedded interpreter harnesses + wire protocol
- `076e9fc` — CP-2 scope paths, meta, env parsing, detached spawn, timeout-escalating client (+ F-1..F-4 closed)
- `ae19dfa` — spec amendment: distinct exit code 7 for protocol mismatch
- `ddadf81` — CP-3 CLI verbs, scope-union resolution, eight-code exit contract (+ fix round, F-5 closed)
- `b6c529d` — CP-4 `[repl] idle_timeout` at repo+user scope with doctor validation
- `e7d8c50` — CP-5 reference docs + discoverability wiring
- `2a640a3` — CP-6 real-binary e2e, start-time idle_timeout warning (F-8), drift guard (F-9)
- `5881b1a` — documentation-surfaces table row

**Out-of-scope work performed during this build:**

- Exit code 7 (protocol mismatch) — CP3 review exposed that sharing 5 breaks code-routing; spec amended mid-loop.
- Start-time stderr warning for invalid `idle_timeout` (F-8) — CP4 review, graphignore precedent; spec amended.
- Node harness materializes as `.mjs` via `HarnessFilename` — a `.js` file would inherit an unrelated ancestor `package.json` `type`.

**Unforeseens — surprises that emerged during implementation:**

- Python idle watchdog only ran in the accept-timeout branch — steady traffic never evaluated the window; found fixing F-1.
- `ps`-based liveness counted zombies as live — caught by a real-harness test; state `Z` now rejected.
- Node `util.inspect` elides long strings at 10k chars, so a string value cannot reach the 64 KiB cap — documented divergence, frame still bounded.
- e2e leak window: cleanup registered after response decode orphaned a live harness on decode failure — reviewer proved by fault injection; guard now registers pre-spawn, keyed off the on-disk meta.

**Deferred items still open:**

- none — all 9 loop findings closed (F-6 fixed post-loop at user's direction, `21e1f40`).
