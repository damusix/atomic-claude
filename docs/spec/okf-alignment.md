...
| 4 | producer: signals-inferrer B3 emits `type: Knowledge` + `description` + bundle-relative concept links; W4 repo-summary output unchanged | `agents/atomic-signals-inferrer.md` + `make render`/`make bundle` | atomic-implementer (feature) | ~3 | prompt declares the contract; bundle parity; render+bundle committed |
