...
2. The replaced section is ≤ 48 lines and the file's net line delta vs commit `6e7fb3c` is ≤ +22 (`git diff --numstat 6e7fb3c -- output-styles/atomic.md`).
...
- 2026-07-03 — **Superseded** SC2 budgets (36→48 section lines, +10→+22 net): user feedback after the first landing — every route now carries a micro-example per the design's revised pinned block. Contract 1a (verbatim landing) unchanged.
- 2026-07-03 — implemented (amendment): `319c12d` — revised block landed byte-identical (46-line section, net +20). Reviewer PASS, zero findings; fence balance, reference-doc consistency, bundle/render parity, and `go test` all re-verified.
- 2026-09-06 — **Superseded** the routing intro sentence only: "compose several per reply, labeled summary bullets first" became "compose shapes only when the content has more than one", as part of the output-style trim (910 → 669 words). The ten route lines and the trailing TUI/Mermaid line are unchanged; contract 1a otherwise stands.
