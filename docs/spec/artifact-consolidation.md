...
- [ ] `agents/` contains exactly 5 `atomic-*.md` files: `atomic-implementer`, `atomic-reviewer`, `atomic-investigator`, `atomic-strategist`, `atomic-signals-inferrer`.
...
| `/refresh-signals` stale after bulk artifact removal | Low | CP8 explicitly runs `/refresh-signals`; `atomic doctor` check 9 (signals freshness) catches drift |
