...
### 2026-08-08 — category 13 validates `[repl] idle_timeout`

**What changed:** Category 13 (`repo-config`) gains `[repl] idle_timeout` validation, per CP4 of `docs/spec/atomic-repl.md`. A present-but-invalid value — unparseable by `time.ParseDuration`, or zero/negative — reports WARN naming the offending value via the shared `config.ValidateIdleTimeout` validator. Absence is normal (no `[repl]` table at all produces no warning); a valid value produces no warning either. The user-scoped `config` check (category 9) validates the same key at `~/.atomic/config.toml` and FAILs on the same invalid shapes — the severity split matches the existing pattern (repo config is optional and never blocks the repo; user config known-key values are FAIL, mirroring `harness.dir` and `[claude.agents].effort`).
