...
### 2026-08-15 — Degenerate tokens are never linked

**What changed:** A third filter joins the skip-set and the gitignore layer: a token built entirely from `.` and `/`, empty token included, is left as plain text. Disk resolution cannot reject these — `join(base, ".")` and `join(base, "/")` are the base directory and `".."` is its parent, so all three stat clean and linkify pointed the reader at the repo root or above it.
