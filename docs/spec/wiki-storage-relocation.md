...
- [ ] `atomic-wiki-inferrer` (agent, current name preserved) writes `docs/wiki/index.md` with OKF frontmatter (`type: Index`, `description:`), plus machine-managed `<wiki-type>repo</wiki-type>`, `<scan-sha>…</scan-sha>`, and `<wiki-schema>1</wiki-schema>` control blocks.
...
|------|-----------|-----------|
| `docs/wiki/` falls under an existing `.gitignore` pattern and scan output is silently untracked | Med | CP6: verify `git status docs/wiki/` after first `/refresh-wiki` run; add `!docs/wiki/` negation if any parent pattern matches |
...
**What changed:** CP4's signals-gate staging now includes `docs/wiki/scan.md` alongside the router and domain files (`git add docs/wiki/*.md`); the prior exclusion (`git restore --staged docs/wiki/scan.md`) is removed from every refresh flow (signals-gate, `/refresh-wiki`, `/autopilot`, `/subagent-implementation`).

**Why:** the drift-scope machinery already treats the committed `scan.md` as the canonical diff baseline — `git diff HEAD -- docs/wiki/scan.md` and the `<scan-sha>` tiebreaker both require it to be in git. Leaving it un-staged made those paths permanently degrade to the git-unavailable fallback. `scan.md` is not `@-ref`'d, so committing it has zero context cost. Filed as GitHub issue #126: "scan.md must go into git. This is how the next guy can build on your work."

**Superseded:** the CP4 row's "not `scan.md`" staging exclusion and the risk-table row "`scan.md` (thousands of lines) accidentally staged by the signals-gate update" — both described the exclusion as intended behavior; committing `scan.md` is now the intended behavior, and the corresponding risk no longer applies.
