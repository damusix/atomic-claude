...
One refresh verb (`/refresh-wiki`), one inferrer agent (`atomic-wiki-inferrer`), and one skill (`atomic-wiki`) operating as a skill-router that dispatches repo vs. realm scope by reading `<wiki-type>`. All dispatch sites, help-router rows, reference tables, and `setup-wiki` scope detection updated to match.
...
- Non-repo realm ("git-wiki") handling in `setup-wiki` scope detection — deferred.
...
5. `templates/commands/setup-wiki.md` writes `<wiki-type>repo</wiki-type>` or `<wiki-type>realm</wiki-type>` into `claude.local.md` (if it exists) else `CLAUDE.md`, using the three-case detection rule: not-a-repo + `wiki/` present → `realm`; repo + no `wiki/` → `repo`; repo + `wiki/` present → ask (treat as `repo` if user cancels or is ambiguous).
...
| 5 | Wire `setup-wiki` scope detection to write `<wiki-type>` | `templates/commands/setup-wiki.md`, rendered `commands/setup-wiki.md` | atomic-implementer surgical | 2 | SC 5 — detection logic present with three-case rule; writes into `claude.local.md` when it exists, else `CLAUDE.md` |
...
| `setup-wiki` asks on `repo + has wiki/` but user cancels; `<wiki-type>` is never written | Low | Future inferrer calls cannot detect scope | Default to `repo` on ambiguous cancel and write it; note this decision in the command output |
