...
`atomic bus` lets concurrent Claude Code sessions on one machine message each other over named rooms, over a per-user daemon that auto-spawns on first use and idle-shuts-down when nothing is subscribed. The `atomic-bus` skill wraps `join` + a `recv --follow` Monitor and carries the addressed-vs-FYI reaction policy. Run `atomic bus --help` for full usage. See [bus reference](/reference/bus) for the room model, envelope shape, and daemon lifecycle.
...
| `atomic bus recv <room> [--follow] [--since <id>] [--json]` | Receive messages. `--follow` streams one JSON envelope per line until SIGTERM. |
...
| `atomic bus serve [--idle-shutdown-minutes N] [--stop]` | Run the daemon in the foreground. `--stop` retires a running one (also the remedy for a protocol version mismatch). |
| `atomic bus tail [<room>] [--all-rooms] [--only-addressed] [--from <name>] [--since <id>] [--json]` | Watch a room's traffic without joining; never appears in `who`. |
