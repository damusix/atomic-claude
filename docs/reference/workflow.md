...
- **Independent review** — code the main agent wrote itself gets an `atomic-reviewer` pass before the commit lands, so the one implementation path with no loop around it still meets a reviewer
...
✓ = runs automatically. ✓* = staleness-gated; skipped for docs-only commits. ✓† = the review gate, which runs once per change: skipped for docs-only commits, for work the implement loop already reviewed, and for a change this gate already read earlier in the same flow. A merge that skips straight to ship with nothing staged has no commit step to gate; there `atomic-verify` carries the review, and `/review-branch` is the explicit pass over `<base>..HEAD`.
