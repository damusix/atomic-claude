...
- **Independent review** — code the main agent wrote itself gets an `atomic-auditor` pass before the commit lands, so the one implementation path with no loop around it still meets a reviewer
...
Every `/commit` invocation runs the review gate, the wiki staleness check, and doc-impact checks as part of the commit flow. The review gate dispatches `atomic-auditor` on the working tree (`diff: working`) and reports its findings and the model that ran before the commit message is written; it skips a docs-only commit, and it skips work that came out of `/implement`, `/subagent-implementation`, `/quick-fix`, `/autopilot`, or `/subagent-diagnose`, which review every checkpoint or iteration already, and a change `atomic-verify`'s gate already audited in this session. Documentation surfaces are presented for review, and the commit message is synthesized from the diff. The wiki is regenerated only when the check reports stale and the staged set is not docs-only; a fresh index (say, because the implement loop already refreshed it) makes the step a no-op. Escalation paths that touch the base branch (`merge`, `squash merge`) also run `atomic-verify` on the merged tip before finalizing.
...
✓ = runs automatically. ✓* = staleness-gated; skipped for docs-only commits. ✓† = the review gate, which runs once per change: skipped for docs-only commits, for work the implement loop already reviewed, for a change `atomic-verify`'s gate already audited this session, and for a change this gate already read earlier in the same flow. A merge that skips straight to ship with nothing staged has no commit step to gate; there `atomic-verify` carries the review, and `/review-branch` is the explicit pass over `<base>..HEAD`.
