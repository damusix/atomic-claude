...
| `atomic-verify` | "done", "fixed", "passing", "ready to merge" | Runs verification before letting Claude claim completion. No evidence, no claim. On code Claude wrote directly, outside the implement loop, verification includes an `atomic-reviewer` pass on the session's model, and the claim states its verdict as `review: <verdict> (atomic-reviewer on <model>)`. |
