...
| `atomic-auditor` | Final gate on a finished implementation, dispatched once after the loop goes green. Audits four things per-checkpoint review cannot see: success criteria no single checkpoint owned, iterations that each passed and do not compose, commit types that misstate user-visible impact, and documentation that is current but says nothing. Never edits the repo; findings also land in `$SCRATCH/AUDIT.md`. Fresh context, ends with PASS or CHANGES_REQUESTED. Also the independent reader for code the main agent wrote outside any loop: `atomic-verify` and the ship verbs' review gate dispatch it with `diff: working`, once per gate event, on the session's model. | caller's choice, `max` effort |
