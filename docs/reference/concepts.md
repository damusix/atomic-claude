...
| **OMP** | Import-free profile `AGENTS.md` (Atomic steering + output-style body) | the discovered extension module at the profile agent root; the package at `~/.atomic/packages/omp/atomic` is a corpus store whose install, registration, and artifact discovery are `unsupported` on the tested version | native cards published where the capability record proves delivery; static scope is advisory today |
