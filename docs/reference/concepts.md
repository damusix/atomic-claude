...
A wiki holds more than code. Register a loose-material folder (research, raw dumps, ticket exports) as a *capture bucket* with `atomic wiki bucket add`, and `/refresh-wiki` fingerprints what changed and synthesizes it into `wiki/knowledge/` pages with cited provenance. The realm verbs are `atomic wiki scan`/`stale`/`linkify` plus `wiki bucket {add,list,diff,promote}`. Concept pages carry an OKF `type` (`Concern`, `Knowledge`), so `atomic serve` renders the whole realm as a typed, navigable graph. See the [knowledge base guide](/guides/knowledge-base).
