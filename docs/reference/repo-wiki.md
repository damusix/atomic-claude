...
- **Body** — the router's one-line domain description, then a typed pointer index: `Map:` (exactly one link, to the domain's wiki page), plus whichever of `Contracts:`, `Reference:`, `Guides:`, `Research:`, `Design:`, `Related:` have at least one link, each checked to exist on disk and sorted by path. A fixed closing line ends every card verbatim, and the whole body caps at 12 lines so an unchanged domain regenerates byte-identical.
