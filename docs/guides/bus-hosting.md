...
With both machines configured, each holding its own key under the `web-api` label, every
`atomic bus` verb works exactly as it does locally, with one addition: `--host <name>` picks the
remote instead of the local daemon.
