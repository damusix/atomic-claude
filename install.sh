...
echo ""
echo "To install the artifact bundle into a coding harness, run one of:"
echo ""
echo "    atomic install --harness claude    # Claude Code"
echo "    atomic install --harness omp       # Oh My Pi"
echo ""
echo "That enrolls the selected harness instance and converges it to what that"
echo "harness actually loads: Claude Code gets AGENTS.md steering, agents,"
echo "commands, skills, output-styles, and rules plus session-start hook"
echo "registration; OMP gets its profile AGENTS.md and the generated extension"
echo "module under the profile's agent root, while its package tree stays Atomic's"
echo "corpus store — package install, registration, and artifact discovery are"
echo "unsupported on the tested OMP version and reported as such. Use --dry-run to"
echo "preview and --instance <root> to name a non-default target."
echo ""
echo "    atomic harness list                # discovered vs enrolled instances"
echo ""
echo "For the Claude-only bundle path (no enrollment), use:"
echo ""
echo "    atomic claude install"
echo ""
echo "To install only signals / reminders helpers without touching a harness,"
echo "skip the above."
