...
|---|---|---|
| **Wiki realm browser** | `atomic serve [path] [--port N] [--open]` starts a local read-only HTTP server that renders the wiki realm (or a single repo) as a navigable, Obsidian-style graph: a page view with a live right rail (this-page graph, out/in links), a whole-system graph toggle, a code-file modal (highlighted source + code intelligence), an `md\|code` search box, and federated code search. localhost only, no auth, no write operations. | [serve](docs/reference/serve.md) |
