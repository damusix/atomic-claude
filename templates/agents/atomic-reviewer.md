...
6. **Required-sections pass**: verify `## Change tree` exists — one line per node, A/M/D markers, covers the files named in the checkpoints, stays sketch-level (no signatures, no algorithms). Verify `## Outline` exists — per file, named pieces with a one-line responsibility each, hollow (no signatures, no bodies, no algorithms), or an explicit `None — <reason>` when the change has no nameable pieces. Verify `## Flows` exists — numbered actor → step sequences per behavior, or an explicit `None — <reason>` when the change ships no runtime behavior. Missing section, unmarked tree, signature-bearing or responsibility-free outline, or vague flows → finding.
