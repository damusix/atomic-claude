...
- **Repo scope** — execute Steps 1-9 (scan → infer → write `docs/wiki/` files → wire `@-ref`). Use incremental or full mode as determined by `first_run` and prior state. Sub-dispatch domain writers (`general-purpose`) and reviewer (`atomic-reviewer`) as described in the reference.
